...
Here \lstlisting{zipWith} is forced to proceed sequentially, as the elements of
\lstlisting{as} must be evaluated sequentially. However, any potential
parallelism in \lstlisting{f} and \lstlisting{bs} would still be exploitable,
...
\lstlisting{Reifiable a S.Exp}. So the most inclusive type would be
\lstlisting{sequential :: Reifiable a S.Exp -> a -> a}. However, we do not take
...
\lstlisting{sequential}. Our initial idea was to add yet another type variable
to Nikola expressions, such that the "sequentiality" of a term would be encoded
in the type. The types of \lstlisting{map},\lstlisting{zipWith} etc. should
...
With the invention of \lstlisting{sequential}, Repa could enjoy a simpler
...
\lstlisting{sequential} construct as a type parametrised monad. But this
