...
\begin{algorithm}
 ...
 % max(F_t, E_t)$
  ...
  \caption{Least Squares Monte Carlo algorithm for American put
    options. $\overbar{S_t}$ and $\overbar{V_t}$ selects those paths
...
The remaining element of the Longstaff \& Schwartz algorithm is the
least squares regression algorithm. 

\todo{Specify the problem}

\todo{Show that it can be solved as a linear system}

\todo{Describe possible linear system solvers, and say that it was kind of
arbitrary that we settled on Cholesky.}

\todo{Describe Cholesky factorization}
