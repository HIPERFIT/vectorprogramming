...
\todo[noinline]{new appendix of benchmark script ''how to'', use
  README from github, and add footnote here} that executed each
...
overhead is insignificant for our purposes.
...
parallelized, using 32 cores\todo{or what?}.

The results are shown in Figure \ref{fig:binomial-cpu}.
\todo{What do we see on the graph (results pending)}

\todo{The overhead of not reusing memory is not that big for these
  problems. Perhaps we are saved by the GC liveness analysis?}
...
iteration \todo[noinline]{this could quickly be changed}, in all other aspects
