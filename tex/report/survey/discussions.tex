...
\todo{Can we find a more precise heading?}
...
Every single resource must be explicitly managed -- there is not even
a function call stack\todo[noinline]{There is in CUDA 5.0 (max 20
  levels of nesting)}. While this enables the coding of very fast
algorithms, programs are also more tightly coupled with the parameters
of the hardware, and to assumptions about input size. So even though a
CUDA programmer has access to highly optimised linear algebra
libraries such as CUBLAS \cite{CUBLAS2013}, optimisations such as loop
fusion must be coded by hand, outruling the use of such
...
and error prone effort for us. \todo{because of ?}
...
\todo{Integration with other CUDA libraries by exporting CUDA device
  points as \texttt{UnboxedForeign} Arrays}
