...
__device_func__(float __internal_fminf(float a, float b))
{
    ...
    if ((cvta.i << 1) > 0xff000000) {
        return b;
    }

    if ((cvtb.i << 1) > 0xff000000) {
        return a;
    }
...
__device_func__(float __cuda_expm1f(float a))
{
    ...
    if (a != 0.0f) {
        u = u * z + a;    // preserve -0 generated by FTZ
    }

    if (t == 128.0f) {
        u = u + u;    // work around intermediate overflow
    }
...
__device_func__(float __cuda_lgammaf(float a))
{
    ...
    if (a == i) {
        return CUDART_INF_F;    /* a is an integer: return infinity */
    }
...
__device_func__(long long int __internal_llroundf_kernel(float a))
{
    ...
    if ((int)ia < 0) {
        res = (unsigned long long int)(-(long long int)res);
    }
