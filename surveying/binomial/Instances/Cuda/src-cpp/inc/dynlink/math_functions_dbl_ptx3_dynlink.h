...
__device_func__(double __cuda_fmax(double a, double b))
{
    ...
    if ((cvtb.l << 1) > 0xffe0000000000000ULL) {
        return a;
    }

    if ((cvta.l << 1) > 0xffe0000000000000ULL) {
        return b;
    }
...
__device_func__(double __cuda_atan2(double a, double b))
{
    ...
    if (t3 == 0.0 && t1 == 0.0) {
        ...
        if (__cuda_fabs(a) > __cuda_fabs(b)) {
            t3 = CUDART_PIO2 - t3;
        }
...
__device_func__(double __cuda_lgamma(double a))
{
    ...
    if (a == i) {
        return CUDART_INF;    /* a is an integer: return infinity */
    }
