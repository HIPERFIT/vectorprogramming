...
binom :: Int -> F
binom expiry = V.head $ CV.toHostVector binom'
  where
    binom' :: CV.Vector F
    binom' = NH.compile (Imp.binom expiry)

-- binomCompiled :: Int -> F
-- binomCompiled expiry = V.head $ CV.toHostVector binomCompiled'
--   where
--     binomCompiled' :: CV.Vector F
--     binomCompiled' = $(NTH.compileSig (Imp.binom (fromIntegral expiry)) (undefined :: Int32 -> CV.Vector F))
